...
## WorkingDogTT fork notes

This fork keeps the local workspace as the source of truth and extends the
upstream `virtuoso-bridge-lite` flow with a more complete agent-oriented setup:

- Remote and local Virtuoso workflows are documented end-to-end, including
  jump hosts, multi-profile `.env` configuration, stable port assignment, and
  first-time setup checks in [`AGENTS.md`](AGENTS.md).
- The CLI surface includes bridge lifecycle checks plus focused Virtuoso helpers
  such as `windows`, `snapshot`, `screenshot`, and `dismiss-dialog`.
- The Python package includes higher-level schematic, layout, Maestro, and
  Spectre utilities, with examples and reference notes under `examples/` and
  `skills/`.
- Windows-friendly setup notes and scripts are included for agent skill linking
  and symlink repair.
